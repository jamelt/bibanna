...
const viewMode = useViewPreferences<"list" | "table">(
  "projectEntries",
  "table",
);
...
async function bulkAction(action: string, extra: Record<string, unknown> = {}) {
  ...
  try {
    const result = await $fetch<{ affected: number; action: string }>(
      "/api/entries/bulk",
      {
        method: "POST",
        body: {
...
const modalAllSelected = computed({
  get: () =>
    ...
    filteredAvailableEntries.value.every((e) =>
      selectedEntryIds.value.has(e.id),
    ),
  set: (value: boolean) => {
    if (value) {
      filteredAvailableEntries.value.forEach((e) =>
        selectedEntryIds.value.add(e.id),
      );
    } else {
      filteredAvailableEntries.value.forEach((e) =>
        selectedEntryIds.value.delete(e.id),
      );
    }
  },
});
  ...
  <div class="space-y-6">
    ...
    <div v-else class="space-y-6">
      ...
      <div class="space-y-3">
        ...
        <!-- Filter bar -->
        <div
          class="flex flex-col lg:flex-row gap-2 items-stretch lg:items-center"
        >
        ...
        >
          <div class="flex flex-wrap items-center gap-2">
            <span
              class="text-sm font-medium text-primary-700 dark:text-primary-300 mr-2"
            >
            ...
            <div class="flex flex-wrap gap-1.5">
              ...
              <UButton
                ...
                :loading="isBulkProcessing"
                @click="
                  bulkAction('removeFromProject', { projectId: project?.id })
                "
              >
                Remove from Project
              </UButton>
        ...
        <!-- Empty state -->
        <UCard
          v-if="entries.length === 0 && !entriesPending && !hasActiveFilters"
          class="py-10"
        >
        ...
        <UCard
          v-else-if="entries.length === 0 && hasActiveFilters"
      ...
      <template #content>
        <UCard>
          ...
          <p class="text-gray-600 dark:text-gray-400">
            ...
            >? This action cannot be undone. All entries will remain in your
            library but will be removed from this project.
          </p>
      ...
      <template #content>
        <UCard>
          <template #header>
            <div class="flex items-center justify-between">
              <div>
                ...
                >
                  {{ selectedEntryIds.size }} of
                  {{ availableEntries.length }} selected
                </p>
              </div>
              <UButton
                icon="i-heroicons-x-mark"
          ...
          <div v-else class="space-y-4">
            ...
            <!-- Entries list -->
            <div
              v-if="filteredAvailableEntries.length === 0"
