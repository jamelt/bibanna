...
<template>
  <div class="h-[calc(100vh-4rem)]">
    ...
    <div v-else-if="!project" class="flex flex-col items-center justify-center h-full">
      ...
      <NuxtLink to="/app/projects" class="mt-2 text-primary-600"> Back to projects </NuxtLink>
    </div>

    <!-- Upgrade prompt for free users -->
    <div v-else-if="!hasFeature('mindMaps')" class="flex items-center justify-center h-full p-6">
      <UCard class="max-w-lg">
        <div class="text-center">
          <div
            class="w-16 h-16 mx-auto rounded-full bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center"
          >
          ...
          <p class="mt-2 text-gray-500">
            Visualize relationships between your sources, authors, and topics with interactive mind
            maps.
          </p>
    ...
    <div v-else class="h-full flex flex-col">
      <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        <div class="flex items-center gap-3">
          <NuxtLink :to="projectDetailPath" class="text-gray-400 hover:text-gray-600">
            <UIcon name="i-heroicons-arrow-left" class="w-5 h-5" />
          </NuxtLink>
          <div>
            <h1 class="text-lg font-semibold text-gray-900 dark:text-white">Mind Map</h1>
      ...
      <div class="flex-1 overflow-hidden">
        <MindmapMindMapViewer :project-id="projectId" />
      </div>
    </div>
  </div>
</template>
