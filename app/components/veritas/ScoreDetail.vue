...
const {
  ...
  pending,
  refresh,
} = await useFetch(`/api/entries/${props.entryId}/veritas`, {
  lazy: true,
})
  ...
  <UCard>
    <template #header>
      <div class="flex items-center justify-between">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Veritas Score</h3>
        <VeritasScoreBadge v-if="score" :score="score.overallScore" size="lg" />
      </div>
    </template>
    ...
    <div v-else-if="score" class="space-y-6">
      ...
      <div>
        <div class="flex justify-between text-sm mb-2">
          ...
          <span class="font-medium text-gray-900 dark:text-white"
            >{{ score.overallScore }}/100</span
          >
        </div>
        <UProgress :model-value="score.overallScore" :color="getScoreColor(score.overallScore)" />
      </div>
      ...
      <UAlert v-if="score.userOverride" color="blue" icon="i-heroicons-information-circle">
        <template #title> Score overridden to {{ score.userOverride }} </template>
      ...
      <!-- Factor breakdown -->
      <div class="space-y-4">
        <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300">Score Breakdown</h4>

        <div v-for="factor in sortedFactors" :key="factor.name" class="space-y-1">
      ...
      <UCard>
        ...
        <div class="space-y-4">
          <p class="text-sm text-gray-500">
            Override the automatically calculated score if you have additional information about
            this source's credibility.
          </p>

          <UFormField label="New Score (0-100)">
            <div class="flex items-center gap-4">
              <USlider v-model="overrideScore" :min="0" :max="100" class="flex-1" />
        ...
        <template #footer>
          <div class="flex justify-end gap-3">
            ...
            <UButton color="primary" @click="handleOverride"> Save Override </UButton>
          </div>
        </template>
      </UCard>
    </template>
