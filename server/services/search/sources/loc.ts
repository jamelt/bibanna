...
async function fetchLocResults(
  ...
  try {
    const response = await fetch(
      `${LOC_API}/search/?${params.toString()}`,
      {
    ...
    return results
      .map((item) => locItemToSuggestion(item))
...
function locItemToSuggestion(item: any): EntrySuggestion | null {
  const title =
    item.title ||
    (Array.isArray(item.title_display)
      ? item.title_display[0]
      : item.title_display);
  if (!title) return null;

  const contributorNames: string[] = Array.isArray(item.contributor)
    ? item.contributor
    : [];
  ...
  const authors: Author[] = contributorNames.map((name: string) => {
    ...
    if (spaceParts.length === 1)
      return { firstName: "", lastName: spaceParts[0] || "Unknown" };
  ...
  const originalFormat = Array.isArray(item.original_format)
  ...
  const metadata: EntryMetadata = {
    ...
    publisher: Array.isArray(item.contributor)
      ? undefined
      : item.contributor,
