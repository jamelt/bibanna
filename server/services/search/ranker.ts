...
function computeScore(query: string, suggestion: EntrySuggestion, field: FieldQualifier): number {
  ...
  switch (field) {
    ...
    case 'publisher': {
      const pubMatch = textSimilarity(query, suggestion.metadata?.publisher ?? '')
      return pubMatch * 40 + citationScore + sourcePriority
    }
    case 'journal': {
      const journalMatch = textSimilarity(query, suggestion.metadata?.journal ?? '')
    ...
    case 'year': {
      const yearMatch = suggestion.year && query.includes(String(suggestion.year)) ? 50 : 0
...
function deduplicateSuggestions(suggestions: EntrySuggestion[]): EntrySuggestion[] {
  ...
  for (const s of suggestions) {
    const doiKey = s.metadata?.doi ? `doi:${normalizeDoi(s.metadata.doi)}` : null
...
function buildFuzzyKey(s: EntrySuggestion): string {
  ...
  const firstAuthor = s.authors[0]?.lastName?.toLowerCase().replace(/[^a-z]/g, '') ?? ''
  const year = s.year ?? ''
...
function authorRelevance(
  ...
  for (const author of authors) {
    const fullName = `${author.firstName} ${author.lastName}`.toLowerCase().trim()
    const nameParts = new Set(fullName.split(/\s+/).filter((w) => w.length > 1))
