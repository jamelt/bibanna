...
export function sortEntries(entries: Entry[], sortBy: string): Entry[] {
...
export async function generateBibliographyHtml(
  entries: Entry[],
  opts: PdfExportOptions,
): Promise<string> {
  const lineHeight =
    opts.lineSpacing === 'single' ? 1.2 : opts.lineSpacing === 'oneAndHalf' ? 1.5 : 2
