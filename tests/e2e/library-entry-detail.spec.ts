...
test.describe('Library Entry Detail Page', () => {
  ...
  test('should display entry details correctly', async ({ page }) => {
    ...
    await expect(page.getByText('This is a test abstract for the entry detail page tests.')).toBeVisible()
  })
  ...
  test('should toggle favorite status', async ({ page }) => {
    ...
    const favoriteButton = page.getByRole('button').filter({ has: page.locator('[class*="i-heroicons-star"]') })
  ...
  test('should add annotation', async ({ page }) => {
    ...
    await expect(page.getByText('This is a test annotation for the entry.')).toBeVisible({ timeout: 5000 })
